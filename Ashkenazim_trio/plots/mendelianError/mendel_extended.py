...
def readPredictFile(gtMatrix, predictFile, minQ):
    samples = []
            ...
            if line[0] == '#':
                if line[1] != '#' and len(samples) == 0:
                    ...
                    for s in line.split()[9:]:
                        ...
                        i += 1
                continue
            formatString = line.split()[8]
            gqidx = getIndexInFormat(formatString, "GQ")
...
if len(argv) != 4:
    print 'Usage: ' + argv[0] + ' <predicted.vcf> <ped.txt> <minQ>'
    exit(1)
predictFile = argv[1]
pedFile = argv[2]
minQ = int(argv[3])
...
gtMatrix = []
sampleMap = readPredictFile(gtMatrix, predictFile, minQ)
