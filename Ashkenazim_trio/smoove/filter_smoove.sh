...
zcat $VCF | grep -e "SVTYPE=DEL" -e "^#" | grep -v "^X" | grep -v "^Y" | grep -v "^h" | grep -v "^G" | grep -v "^N" > $FILTERED
awk -F '[\t;=]' '{if ($13-$2 >= 500 && $13-$2 <= 10000 || $1 ~ /^#/ ) print $0}' $FILTERED > $SIZE
...
cat t tt > $HG002Only
rm t tt
awk -F '[\t;=]' '{if ($13-$2 >= 500 && $13-$2 <= 10000 || $1 ~ /^#/ ) print $0}' $HG002Only > $HG002Only_SIZE
